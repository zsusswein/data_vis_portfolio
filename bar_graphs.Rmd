...
## Data
The data are from the National Survey on Drug Use and Health , which is conducted annually by the Substance Abuse and Mental Health Services Administration (SAMHSA), an agency in the U.S. Department of Health and Human Services (DHHS).  More information about this survey can be found at [ICPSR's data archive page](http://www.icpsr.umich.edu/icpsrweb/ICPSR/series/64).
...
# Data
load("36361-0001-Data.rda") # available in the github repo
data <- da36361.0001
remove(da36361.0001)
```
...
heroin_user <- filter(data, HEREVER == "(1) Yes")
non_user <- filter(data, HEREVER == "(2) No")

#Combining data
heroin_user$user_status <- "Heroin User"
non_user$user_status <- "Non User"
combined_data <- rbind(heroin_user,non_user)

# Take percentages in each income bracket across heroin status
summary_data_non_user <- summarize(group_by(non_user, IRPINC3, user_status),
                                count=n())
summary_data_non_user$percent <- summary_data_non_user$count/sum(summary_data_non_user$count)

summary_data_heroin_user <- summarize(group_by(heroin_user, IRPINC3, user_status),
                                  count=n())
summary_data_heroin_user$percent <- summary_data_heroin_user$count/sum(summary_data_heroin_user$count)

summary_data <- rbind(summary_data_non_user, summary_data_heroin_user)
```
...
ggplot(combined_data)+
  geom_bar(aes(x=user_status,fill=IRPINC3),position="fill")+
...
```{r bar, echo = TRUE}
#Bar Chart
bar_chart_heroin_user <- 
  ggplot(heroin_user)+
  geom_bar(aes(x=IRPINC3,fill=IRPINC3))+
  coord_flip()+
...
bar_chart_non_user <- 
  ...
  geom_bar(aes(x=IRPINC3, fill=IRPINC3))+
  coord_flip()+
  labs(x='Non-users', y='Number of respondents')+
  scale_fill_brewer(palette="RdYlBu",direction=1,
                    name="Income Level")+
  theme_minimal()
